...
@optionalTypeArgs
abstract class BaseImageProvider<T extends Object> {
  ...
  ImageStream resolve(ImageConfiguration configuration) {
    ...
    _createErrorHandlerAndKey(
      configuration,
        ...
        assert(() {
          collector = () sync* {
            yield DiagnosticsProperty<BaseImageProvider>('Image provider', this);
  ...
  /// [ImageCache].
  @protected
  void resolveStreamForKey(ImageConfiguration configuration, ImageStream stream, T key, ImageErrorListener handleError) async{
    ...
    //使用异步完成图片数据获取
    ImageStreamCompleter _imageStream = await load(key, PaintingBinding.instance.instantiateImageCodec);
    final ImageStreamCompleter completer = PaintingBinding.instance.imageCache.putIfAbsent(
      key, () => _imageStream ,
      onError: handleError,
    );
  ...
  @protected
  Future<ImageStreamCompleter> load(T key, DecoderCallback decode);
